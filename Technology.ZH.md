...
重新上线的邻近客户端提供缺失消息的补收——无需接入互联网。这是 FidoNet
术语中「据点」（point）概念最接近的对应物。

### 基础设施中继节点 (`ROUTER` / `ROUTER_LATE`)
通过继续中继数据包来扩大网络的无线覆盖范围。此外还可以接入互联网，充当
不同网络分段之间、或与总干线（backbone）之间的桥梁。
...
任何一个特定节点；而缺失历史消息的补收则由任意可用的基础设施节点提供
服务，而非某一个固定不变的节点。
