...
周期性轮询——避免无谓地空等最多一分钟。在本次轮询中，与这个已经应答过的
节点之间的后续交换是定向（addressed）的，而非广播：针对该节点已经响应过
的信区再次发起的同步请求只发给它一个，并在发送层面进行完整的分轮确认，
而不是再次盲目地向所有人广播。每个信区的第一次请求（勘测轮次，尚无已确
认的联系人）仍然刻意保持广播——历史消息的来源可能不止一个，不同基础设施
节点的复制程度也可能不同。
