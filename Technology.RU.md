...
●CLIENT ◄──LoRa──► ●CLIENT ◄──LoRa──► ■CLIENT_BASE ◄──LoRa──► ●CLIENT (TRACKER/SENSOR/…)
                                          ...
                                          LoRa
                                            │
                                            ▼
                                  ▲ROUTER / ROUTER_LATE ◄──LoRa──► ( Internet-аплинк )
                                                                     мост в другие
                                                                     сегменты сети
...
пропущенных сообщений соседними клиентами, вернувшимися в эфир — без выхода
в интернет. Это ближайший аналог «поинта» в терминологии FidoNet.

### Инфраструктурный узел-ретранслятор (`ROUTER` / `ROUTER_LATE`)
Расширяет радиус действия сети, ретранслируя пакеты дальше. Дополнительно
может иметь выход в интернет и выступать мостом между разрозненными
сегментами сети или в общий бэкбон.
...
| Роль             | Назначение                                                        | Категория |
|------------------|--------------------------------------------------------------------|-----------|
| `CLIENT`         | Обычное клиентское устройство (по умолчанию)                      | клиентская |
| `CLIENT_MUTE`    | Не ретранслирует чужие пакеты                                      | клиентская |
| `CLIENT_HIDDEN`  | Скрытный режим / энергосбережение, минимум эфирной активности      | клиентская |
| `TRACKER`        | Приоритет GPS-позиционирования                                     | клиентская |
| `SENSOR`         | Приоритет телеметрии с датчиков                                    | клиентская |
| `TAK`            | Интеграция с системой ATAK                                         | клиентская |
| `TAK_TRACKER`    | ATAK + автоматические PLI-маяки позиции                            | клиентская |
| `LOST_AND_FOUND` | Регулярно транслирует местоположение — поиск потерянного устройства| клиентская |
| `ROUTER`         | Инфраструктурный ретранслятор, виден в списке узлов сети            | инфраструктурная |
| `ROUTER_LATE`    | Ретранслятор с низким приоритетом — включается последним, покрытие локальных кластеров | инфраструктурная |
| `CLIENT_BASE`    | Персональная базовая станция — узел-кэш для дополучения истории    | инфраструктурная |

Сеть также распознаёт узлы с ролями `ROUTER_CLIENT` и `REPEATER` — более
ранними вариантами инфраструктурной роли, встречающимися на устройствах со
старыми версиями прошивки Meshtastic, и учитывает их наравне с `ROUTER` /
...
узлу, а дополучение пропущенной истории обслуживает любой доступный
