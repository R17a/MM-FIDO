...
![Setup wizard: first name, last name, device selection, language selection](Sccreenshots/s1.EN.png)
...
![Setup wizard: expanded connection method list — Mock mode, real device over USB, Bluetooth (BLE), Network (TCP/WiFi)](Sccreenshots/s2.1.EN.png)
...
![Setup wizard: MAC address field, "Scan" button, found device in a dropdown list, ESP32 warning](Sccreenshots/s2.2.EN.png)
...
![Modal window "Network Policy" with 5 rules and Accept/Decline buttons](Sccreenshots/s3.EN.png)
...
![Area list: System/News, Personal mail, Local folders, RU/EN/ZH echo conferences](Sccreenshots/s4.1.EN.png)
![The same screen after switching to dark theme (F8)](Sccreenshots/s4.2.EN.png)
...
![Table of nearby nodes: roles, hop-count path, internet indicator](Sccreenshots/s5.EN.png)
...
![Log: MQTT bridge startup, topic subscription, packet send/receive](Sccreenshots/s6.EN.png)
...
![Message reading screen: AREA/FROM/TO/DATE/SUBJ header "Re: Test message" and text quoting the original message](Sccreenshots/s8.EN.png)

Reading a reply in the `EN.TALK` echo conference (message 2 of 3 in the area) — area, sender, recipient, date, and subject in the header, with the text below auto-quoting the original message.

### 9. Composing a new message in an echo conference

![New message editor: recipient All, subject "Test message", typing the message text](Sccreenshots/s9.EN.png)

The built-in editor for a new message in the `EN.TALK` echo conference — recipient, subject, and message text.

### 10. Message thread tree

![List of messages in the echo conference with thread tree enabled: a topic, a nested reply to it, and a separate second topic](Sccreenshots/s10.EN.png)
...
![Help: area list, message reading — hotkeys](Sccreenshots/s11.EN.png)
...
![Help: replying and composing messages, built-in text editor — hotkeys](Sccreenshots/s12.EN.png)
