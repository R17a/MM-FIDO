...
![Тот же экран после переключения на тёмную тему (F8)](Sccreenshots/s4.2.RU.png)

Главный экран приложения в стиле GoldED: служебная область `SYS.ANNOUNCE`, личная почта (`NETMAIL`), локальные папки (`ARCHIVE`/`IN`/`OUT`) и стартовый набор эхоконференций (`EN.TALK`, `RU.TALK`, `ZH.TALK`). Второй скриншот — тот же экран после переключения светлой темы на тёмную (клавиша `F8`).
