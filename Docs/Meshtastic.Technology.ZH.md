...
信区分为**全局**和**区域**两类。全局信区——`SYS.*` 和语言信区（`RU.TALK`、
`EN.TALK`）——默认所有人都订阅，且保存在每个节点上。区域信区按
`<国家>.<地区>.<主题>` 命名（`RU.MSK.TALK`、`DE.BER.MEET`）——两个地理层级，
大致相当于经典 FidoNet 的信区标签。地理归属是信区记录本身的一部分，而不仅仅
体现在名称里。

**订阅（`S` 键）通过 HUB 的目录进行**，并设计为两次短请求的级联，以免让
数百个他乡城市的列表在 LoRa 上传输：节点先请求「索引」（国家和地区的列表
——数十行短字符串，不含信区名称），再只请求所选地点的信区名称。勾选的信区
会加入订阅（`+AREA` / `%JOIN`）。无论网络规模多大，LoRa 流量都是数百字节而
非数千字节。对话框不显示全局信区（它们本就已订阅）；范围内没有基础设施节点
的扁平 `CLIENT` 无可浏览——此时对话框被禁用，而基础设施节点则从自己的数据库
显示目录。

**节点关心哪些地区在 `config.json` 中设置（`echo_regions`，首次运行向导的
一步）。** 该过滤**仅对没有互联网的节点生效**：有互联网的节点始终接收并分发
所有地区的完整目录（互联网 HUB 有意承担完整复制）。对仅 LoRa 的节点而言，
空列表意味着「仅全局信区」。没有互联网的 `CLIENT_BASE` 同样只复制自有地区和
全局信区，外加手动订阅的内容。要订阅远方地区，需要一个确实携带这些信区的
HUB（通常是有互联网的 HUB）：本地区的 HUB 通过无线电根本不保存它们。

**无法从订阅对话框创建信区**——用户只能订阅。创建新信区是一个单独的流程，
需由根节点操作员手动批准：签名申请以私信（NETMAIL）发往根节点，操作员批准
或拒绝，签名记录经普通同步在网络中扩散；只有已批准的信区才会进入共享目录。
审核（冻结信区、撤回消息、封禁作者）由同样的签名记录机制驱动——在接收时应用，
不做物理删除。该流程正在开发中。

![信区目录：全局与区域信区、按地理过滤的订阅、经操作员创建信区](images/Meshtastic/catalog-geo.ZH.svg)
...
在两个基础设施节点之间，这种反向交换还会在时间上错开：当一方正在从另一方
拉取历史时，另一方会暂缓自己的反向 push，留到下一个周期再补发。这样两个
基站就不会从两端同时占满半双工无线电信道（在双向猛发下，过去这曾让发送
队列长时间卡住）；「对方正在从我们这里拉取」这个标记会自行过期，若对方
仍有缺失，下一个周期便会补上。
...
这个广播探测本身等待应答的时间很短——大致相当于存在心跳信标的间隔，而不是
按「满载邮件的多分片应答」校准的长超时（对探测的应答始终是单分片）。如果
在这段时间内（其间会有几次探测重发）无人应答，外层轮询循环就会很快再次
尝试，更容易在邻居刚结束发送、空出信道的时刻抓住它。
...
节点如何在时间上分布同一帧的各个分片——一次性全部发出、逐个发送，还是分成
等待确认的小批次——属于发送方的本地设置（纯 LoRa 场景下的现场测试开关）。
它不属于帧格式，也不与对端进行任何协商：接收方无论如何都以相同方式重组分片，
未送达的部分由同一套按轮重试机制补发。因此，采用不同节奏设置的两个节点——
包括每个传输方向使用不同设置——都能不受限制地互相通信。
